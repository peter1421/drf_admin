...
from book_chatbot.models import StudentBookBot
from book_chatbot.serializers.bookbot import StudentBookBotSerializer
...
class StudentBookBotView(APIView):
    def get(self, request, format=None):
        try:
            ...
            student_book_bot = StudentBookBot.objects.get(
    ...
    def put(self, request, user_id, book_id, format=None):
        try:
            student_book_bot = StudentBookBot.objects.get(
                student_id=user_id, book_id=book_id)
            serializer = StudentBookBotSerializer(
                student_book_bot, data=request.data)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
